...
interface RadioTabsProps extends React.ComponentProps<
...
function RadioTabs({
	...
	const hasDescriptions = React.Children.toArray(props.children).some(child => {
		if (
			...
			const itemChildren = React.Children.toArray(
				child.props.children as React.ReactNode
			)
			return itemChildren.some(itemChild => {
				return (
					React.isValidElement(itemChild) &&
					itemChild.type === RadioTabsItemDescription
				)
			})
		}
		return false
...
function RadioTabsItem({
	...
	const hasStructuredChildren = React.Children.toArray(children).some(child => {
		return (
			React.isValidElement(child) &&
			(child.type === RadioTabsItemLabel ||
				child.type === RadioTabsItemDescription)
		)
	})
...
function RadioTabsItemLabel({
	className,
	children,
	...props
}: RadioTabsItemLabelProps) {
	return (
		<span
...
function RadioTabsItemDescription({
	className,
	children,
	...props
}: RadioTabsItemDescriptionProps) {
	return (
		<span
